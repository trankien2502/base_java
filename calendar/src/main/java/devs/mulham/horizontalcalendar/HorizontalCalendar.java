...
//    }
     ...
     *                  ,or false to play default scroll animation speed.
     */
    public void selectDate(Calendar date, boolean immediate, boolean isChoose) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault());
        Log.e("check_date", "date in pick:" + sdf.format(date.getTimeInMillis()));
        int datePosition = positionOfDate(date);
        if (immediate) {
            centerToPositionWithNoAnimation(immediate, datePosition);
            if (calendarListener != null) {
                calendarListener.onDateSelected(date, datePosition, isChoose);
            }
        } else {
